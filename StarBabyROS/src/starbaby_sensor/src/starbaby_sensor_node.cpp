...
void center_range_handler(const sensor_msgs::RangeConstPtr &data) {
	sensor_msgs::PointCloud point_cloud;

	range_to_point_cloud(data, point_cloud);

	center_point_cloud_publisher.publish(point_cloud);
}
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "starbaby_sensor_node");

	ros::NodeHandle nh;

	// Create publishers
	center_point_cloud_publisher = nh.advertise<sensor_msgs::PointCloud>("sonar/center_point_cloud", 1, false);
	...
	// Create subscribers
	center_range_subscriber = nh.subscribe<sensor_msgs::Range>("sonar/center", 1, center_range_handler);
